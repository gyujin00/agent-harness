---
name: "source-command-intake"
description: "requirements/의 프로젝트 문서를 읽어 하네스를 자동으로 부트스트랩한다 (Sprint-01 초안 + ADR-proposed + 도메인 스캐폴드)"
---

# source-command-intake

Use this skill when the user asks to run the migrated source command `intake`.
...
- 새로 만든 spec이 있으면 `python3 scripts/generate_agents.py`를 실행해 `.Codex/agents/`에
  반영하고, `harness/permissions.policy.md` · `.Codex/hooks/enforce_permissions.py`의 POLICY
...
4·5단계 게이트의 멱등성 확인은 `docs/harness-log.md`에서 **이 절차(`/intake`) 자신이 남긴 기록**만
찾는다 — 이 하네스를 부트스트랩하기 전에 사람이 수작업으로 이미 같은 결정을 내려둔 경우(예:
`docs/decisions/`에 이미 `accepted` ADR이 있거나, `harness-log.md`에 서술형으로만 기록된 경우)는
인식하지 못해 같은 질문을 다시 할 수 있다. 안전하지 않은 것은 아니다(중복 생성·덮어쓰기는 나지
않는다) — 다만 이미 답한 질문을 한 번 더 받을 수 있다는 뜻이다. `docs/harness-log.md`의
`intake-idempotency-check` 항목에 실제 재현 사례가 기록돼 있다.
