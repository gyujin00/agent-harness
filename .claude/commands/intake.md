...
## 2. 요구사항 추출
각 문서에서 **"요구사항"/"Requirement(s)"/"Functional Requirement"가 포함된 절 제목(예: "## 3.
Functional Requirements", "## 요구사항") 바로 아래에 있는 표**만 대상으로, 첫 컬럼이
`[A-Z]+-\d+` 패턴(REQ-001, FR-017, US-12 등 접두어 무관)인 행을 요구사항으로 수집한다. 각
항목에 대해 ID·설명·우선순위(있으면)·출처 문서(및 그 표가 속한 절 제목)를 기록해둔다.

**주의**: 같은 문서에 Business Rule(BR-*), Data Requirement(DATA-*), Acceptance Criteria(AC-*),
Dependency(DEP-*), Open Question(OQ-*) 같은 다른 ID 체계의 표가 함께 있을 수 있다 — 이런 표는
절 제목이 "요구사항"이 아니므로 이 단계에서 수집 대상이 아니다. 절 제목만으로 요구사항 절을
특정할 수 없는 문서라면, 사람에게 "이 문서에서 어느 절이 요구사항 목록인지 알려주세요"라고
확인을 요청한다(지어내지 않는다).
